...
#import "ShaderDirectiveCatalog.h"

#import "ShaderDirectiveVocab.h" // vocabulary tables (shares E / Colored / kVAR)
...
// in ShaderParseOSCBlocks: a non-comment line, a blank comment, or a `#`/`@`
...
ShaderDirectiveCompletions(NSString *text, NSUInteger caret,
  ...
  *outReplaceRange = NSMakeRange(NSNotFound, 0);
  ...
  if (slash.location == NSNotFound) {
    ...
    NSMutableArray *pool = [NSMutableArray arrayWithArray:ShaderGLSLIdents()];
    ...
    if ([between stringByTrimmingCharactersInSet:NSCharacterSet
      ...
      NSString *token =
          [text substringWithRange:NSMakeRange(wordStart - 1,
                                               caret - (wordStart - 1))];
      NSArray *items = FilterByPrefix(ShaderDirectiveKinds(), token);
      if (items.count)
  ...
  if (valuePos) {
    ...
    NSArray *pool = ShaderValueEnumForKey(key);
    ...
    if (!pool && inBlock) {
      NSMutableArray *m =
          [NSMutableArray arrayWithArray:ShaderOSCExprBuiltins()];
  ...
  if ([bodyTrim hasPrefix:@"#"])
    pool = ShaderDirectiveAttributeKeys();
  else if (![bodyTrim hasPrefix:@"@"] && inBlock)
    pool = ShaderOSCFieldKeys();
  if (!pool)
    return @[];
  NSArray *items = FilterByPrefix(pool, word);
  if (items.count)
