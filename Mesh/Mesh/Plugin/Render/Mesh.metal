...
fragment float4 fluidFragment(RasterizerData in [[stage_in]],
    ...
    float t = 0.3 * (u.time * u.speed + fmod(u.seed, 10000.0));
    ...
    float2 p = uvn - 0.5;
    p.x *= aspect;
    p = mg_rotate(p, u.rotation);
    p /= max(u.scale, float2(0.01));
...
fragment float4 neonFragment(RasterizerData in [[stage_in]],
                             constant NeonUniforms &u [[buffer(MeshFragmentIndex_Grid)]],
                             constant int &encodeSRGB [[buffer(MeshFragmentIndex_EncodeSRGB)]]) {
    float2 res = max(u.resolution, float2(1.0));
    float aspect = res.x / res.y;
    ...
    float t = u.time + fmod(u.seed, 10000.0);
    float speed = 0.5 * u.speed;
    ...
    float2 uv = uvn - 0.5;
    uv.x *= aspect;
    uv = mg_rotate(uv, u.rotation);
    uv /= max(u.scale, float2(0.01));
...
fragment float4 silkFragment(RasterizerData in [[stage_in]],
                             constant SilkUniforms &u [[buffer(MeshFragmentIndex_Grid)]],
                             constant int &encodeSRGB [[buffer(MeshFragmentIndex_EncodeSRGB)]]) {
    float2 res = max(u.resolution, float2(1.0));
    float aspect = res.x / res.y;

    // Seed offsets the flow phase. 0.4 is the source's default flowSpeed.
    float t = u.time * 0.4 * u.speed + fmod(u.seed, 10000.0);
...
fragment float4 strataFragment(RasterizerData in [[stage_in]],
    ...
    float t = u.time * 0.5 * u.speed + fmod(u.seed, 10000.0);
    ...
    float2 uvn = float2(in.textureCoordinate.x, 1.0 - in.textureCoordinate.y);
    float2 cc = uvn - 0.5;
    cc = mg_rotate(cc, u.rotation);
    cc /= max(u.scale, float2(0.01));
    float2 uv = cc + 0.5;
    uv -= float2(u.origin.x - 0.5, u.origin.y - 0.5);
