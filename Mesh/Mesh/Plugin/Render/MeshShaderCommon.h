...
// Shared Metal helpers used by every Mesh generator Type: the reference-frame
// synthesis, the noise / hash primitives, and the core film-grain overlay.
// Extracted from Mesh.metal so each Type's shader can rely on one place.
...
// (tiling, + 0.5, etc.) is applied by the caller. Mesh (y-down), Dithering and
// Grainy build their own frames.
static float2 mesh_objectUV(float2 tc, float aspect, constant MeshCommonUniforms &cm) {
...
static float mesh_grainSample(float2 guv) {
...
static float3 mesh_applyGrain(float3 color, float2 fragCoord, constant MeshCommonUniforms &cm) {
    float amt = max(cm.grain * cm.grainScale, 0.0);
    float2 guv = fragCoord / max(cm.grainSize, 0.25);
    float gv = mesh_grainSample(guv);
