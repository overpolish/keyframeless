...
## glslang + SPIRV-Cross (Mesh Custom shader transpiler)

The Mesh generator's **Custom** type compiles user-supplied Shadertoy-style GLSL
...
merged static library the Mesh XPC service links.
...
Then build the Mesh **XPC Service** in Xcode as usual. The `build/` output is
...
The Mesh XPC target's build settings point `LIBRARY_SEARCH_PATHS` /
`HEADER_SEARCH_PATHS` at `ThirdParty/build/{lib,include}` and link
`-lkktranspiler`.

- Bridge: `Mesh/Mesh/Plugin/Render/KKGLSLTranspiler.{h,mm}`
