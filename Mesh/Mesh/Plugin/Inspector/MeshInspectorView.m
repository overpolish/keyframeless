...
static NSString *const kMeshIntroSeenKey = @"MeshIntroSeen";

@implementation MeshInspectorView
  ...
  self = [super initWithAPIManager:apiManager
             ...
             maintainTimingEnabled:maintainTimingEnabled
                    ...
                    availableLanes:availableLanes
                          timeline:timeline];
  if (self) {
    _miniViewerRenderer = [[MeshMiniViewerRenderer alloc] init];
...
- (void)viewDidMoveToWindow {
  ...
  if (!self.isDetachedCopy)
    [self autostartIntroGuideOnceWithSeenKey:kMeshIntroSeenKey];
}
