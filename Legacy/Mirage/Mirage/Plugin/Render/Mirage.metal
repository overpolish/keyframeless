...
// (MirageCommon.h) and the 12 built-in Type fragments live on disk as reference
...
struct MirageBlitData {
    float4 position [[position]];
    float2 uv;
};

vertex MirageBlitData meshBlitVertex(uint vertexID [[vertex_id]]) {
...
fragment float4 meshBlitFragment(MirageBlitData in [[stage_in]], texture2d<float> src [[texture(0)]],
