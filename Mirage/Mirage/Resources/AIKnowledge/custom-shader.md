...
- Every Image shader declares its browser/runtime type with exactly one `// #template generator`, `filter`, `layout`, `transition`, or `color-transform` line. Without one the shader still compiles and renders: the editor shows an amber warning under the code (not a red compile error) and the Save button stays disabled until the line is added. Two `#template` lines or an unknown type are real errors.
...
| `iProgress`              | `float`     | clip fraction, `0` at the first frame to `1` at the last, linear. Deliberately NOT scaled by Speed/Seed. In a transition template this is the GL Transitions `progress` and is how a transition should read its sweep - it advances over the item on its own, with nothing to declare |
...
Where the catalogue can be beaten on its own spec: `progress` is linear-only on the web, and the optional `// #progress` directive turns it into a lane on the timing engine, so a transition can be eased, held, or reshaped like any other property. That is an opt-in extra, not the normal way to read the sweep - without it, `iProgress` already runs the transition.
