...
- The keypose, constants, curve and modulation panels stay open while you work elsewhere in Final Cut. Close one with its Close button or Esc. Unpin a panel with the pin button beside Close, or press K, and a click anywhere outside it (other than a companion panel) closes it too; the pin state is shared by all panels and remembered. If you are editing a number field or the property search box, Esc first cancels that edit and leaves the panel open, so a second Esc closes it.
- Drag empty space or the decorative title in the title bar to move a panel. Constants and keypose can be resized from any edge or corner; their shared size is remembered.
...
- Press L, or use the sidebar button beside composition peek, to hide or show Mirage's template browser or Canvas's layer list. It starts shown and remembers the choice while that inspector is open.
- Mirage also shows a right-sidebar button for its grading panel. Press G to hide or show it. Plugins without a right companion panel do not show this control.
- Press K, or click the pin beside Close, to pin or unpin the editor panels. Pinned (the default) they only close from Close or Esc; unpinned, a click outside them closes them. One shared, remembered setting.
- Press V, or use the compact-editor button, to hide or restore the mini-viewer in Constants and Keypose. Compact mode leaves the parameters visible for editing against Final Cut's main composition viewer.
