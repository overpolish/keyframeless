...
The mini-viewer only receives the current clip. To check the whole composition, hold the layered-rectangles button beside Close, or hold P. The editor panel disappears while held so Final Cut's own viewer is visible, then returns to the same keypose, constant, curve, or modulation when released. Editor panels stay open while you work elsewhere in Final Cut; close them with their Close button or Esc. The pin button beside Close, or the K key, changes that: pinned (the default) is this stay-open behaviour, unpinned means a click anywhere outside the panel closes it, exactly like pressing Close, including clicks in the inspector. A click that opens or switches an editor (the Constants button, a keypose cell, a curve segment) switches in place exactly as when pinned rather than closing and reopening. Clicks in a companion panel such as Mirage's browser and grading panel or Canvas's layer list never count as outside. The pin is one shared setting for all panels and is remembered.
