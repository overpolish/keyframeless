...
- (BOOL)properties:(NSDictionary *_Nonnull *)properties
  ...
  *properties = @{
    // YES so FCP calls -scheduleInputs: (needed for filmstrip/onion/boundary
    // multi-frame source). DIAGNOSTIC: investigating whether this is compatible
    // with ChangesOutputSize for the main-viewer render.
    kFxPropertyKey_MayRemapTime : @YES,
    kFxPropertyKey_PixelTransformSupport : @(kFxPixelTransform_ScaleTranslate),
    kFxPropertyKey_VariesWhenParamsAreStatic : @YES,
    // Glow bleeds beyond the source bounds, so the dest image is larger than
    // the source and the blur must see neighbouring tiles.
