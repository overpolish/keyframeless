...
- The **Dynamic** toolbar toggle (Advanced) widens short transitions so they stay easy to grab on a long clip; each lane then shows its own playback line. It's display-only, off by default, and never changes the animation.
