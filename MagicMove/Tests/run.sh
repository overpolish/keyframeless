...
for suite in ${MM_TEST_SUITES:-DefaultsTests HostLifecycleTests HeaderTests LinkedPosesTests ModelTests MatchEndpointsTests CombinedPoseTests EasingTests AddedMotionTests MotionBlurTests ShortcutTests CustomRowTests PositionTests ScaleTests TimingEditorTests OpacityTests RotationTests BlurAnchorTests ResetParameterTests NativeLinksTests PropertyMatchTests OSCWriteTests OSCNudgeTests}; do
  xcrun clang -fobjc-arc -fmodules -Wno-protocol -fsanitize=address,undefined \
    ...
    "$core/MMDefaults.m" "$core/MMInspectorClock.m" "$core/MMOSCPlayheadNudge.m" "$preferences/PPDefaultStore.m" "$inspector/ICContextMenu.m" "$inspector/ICDefaultMenu.m" "$core/MMParameterData.m" "$core/Plugin.m" "$core/Plugin+CustomRow.m" "$core/Plugin+Links.m" "$core/Plugin+Parameters.m" \
    "$core/MMInspectorHeader.m" "$inspector/ICInspectorHeader.m" "$inspector/ICPopUpButton.m" "$inspector/ICMenuToggleView.m" "$inspector/InspectorTokens.m" "$inspector/ICValueTextField.m" "$inspector/ICInspectorRow.m" "$inspector/ICSliderView.m" "$inspector/ICSliderRow.m" "$core/MMPoseTiming.m" "$core/MMTimingEditorModel.m" "$core/MMTimingEditor.m" "$core/MMScalePose.m" "$core/MMScalarPose.m" "$core/MMPropertyLane.m" "$core/MMRotationPose.m" "$core/MMAnchorPose.m" "$core/MMPropertyRow.m" "$core/MMResetParameter.m" "$core/MMMatchEndpoints.m" "$core/MMNativeLinks.m" "$core/MMShortcut.m" "$core/MMCombinedPose.m" "$core/MMDestinations.m" "$render/Plugin+Render.m" "$render/MMRenderHost.m" "$osc/MagicMoveOSC.m" "$viewer/OSCCursor.m" "$viewer/OSCViewerControl.m" "$viewer/OSCViewerControl+Draw.m" "$viewer/OSCViewerControl+Input.m" "$viewer/OSCViewerControl+Rotation.m" "$viewer/OSCViewerControl+Anchor.m" "$support/RenderSupport.m" "$support/RSMetalResources.m" "$support/RSOSCDrawing.m" \
    "$test_tmp/MotionTiming.o" "$test_tmp/MTDurationRecords.o" "$test_tmp/OSCPlayheadMotion.o" "$test_tmp/OSCBoxGeometry.o" "$test_tmp/OSCRotationGeometry.o" -o "$test_tmp/$suite"
