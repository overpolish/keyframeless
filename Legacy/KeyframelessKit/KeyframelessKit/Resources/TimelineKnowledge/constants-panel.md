...
The Constants panel uses the same value-editor popover as keyposes. The only difference is that the value applies to the whole clip. While it's open, the Constants button in the toolbar lights up (tints accent), so you can tell at a glance you're editing constants rather than a keypose.
...
The preview also plays along with the clip: scrub or play back and it follows the playhead with its on-screen controls hidden, so you can watch the animation in motion without closing the panel.
