...
#import <math.h>
#include <stdint.h>
static NSDictionary *KFCopyMotionSettings(NSDictionary *settings) {
...
- (KFPoseTiming *)timingByReplacingMotionSettings:
  ...
  KFPoseTiming *copy = [self timingByReplacingLinkID:self.linkID];
  copy->_motionSettings = KFCopyMotionSettings(settings);
  if (!copy->_motionSettings)
    return nil;
  return copy;
}
- (KFPoseTiming *)timingByReplacingLinkID:(NSString *)linkID {
  KFPoseTiming *copy = [[KFPoseTiming alloc] initWithDuration:self.duration
...
- (KFPoseTiming *)timingByReplacingMotionSeed:(uint32_t)seed
                                       linked:(BOOL)linked
...
- (KFPoseTiming *)timingByCopyingMotionOptionsFrom:(KFPoseTiming *)other {
  if (![other isKindOfClass:KFPoseTiming.class])
    return self;
...
- (instancetype)initWithCoder:(NSCoder *)c {
  ...
  if (self && [c containsValueForKey:@"motionSettings"]) {
    ...
    _motionSettings = KFCopyMotionSettings(settings);
    if (!_motionSettings)
      return nil;
  }
...
- (BOOL)isEqual:(id)obj {
  if (![obj isKindOfClass:KFPoseTiming.class])
    return NO;
  KFPoseTiming *other = obj;
