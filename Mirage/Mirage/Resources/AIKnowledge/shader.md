...
- Saving a shader to the browser tags it with a **category** - one of **Generator** (draws its own look), **Audio** (reacts to a `#audio` binding), **Filter** (processes the clip on `iChannel0`), **Transition** (blends two clips across `iProgress`), or **Layout** (draws a clip into one region - picture-in-picture, split, quarters - and stays transparent elsewhere with `// #alpha`). It's a save-time label, not a code setting: nothing in the shader source changes with it, and Generator is the default. The browser filters by category (the icon pills above the list) and badges each card with its type. Hover a card and click its small pin (bottom-left) to make that template the **default** for new Mirage effects and new rack entries; the current default shows a filled pin and a subtle animated border. A shader that does more than one thing is filed under whichever the author leads with.
