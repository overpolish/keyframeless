...
/// Escape's virtual key code. Hard-coded because it is a hardware constant, not a
/// layout-dependent character.
static const unsigned short kEscapeKeyCode = 53;

/// Remove an event monitor and forget it, tolerating one that was never installed.
/// Six of them arm and disarm together, and a teardown that misses one leaves the
/// panel eating clicks nobody armed it for.
...
static NSString *MirageSampleTitle(MirageMemoryColor kind) {
  ...
  case MirageMemoryColorSkin:
    return RLoc(@"Sample skin", @"Color panel button that samples a patch of skin "
                                @"to measure how far off it is.");
  case MirageMemoryColorFoliage:
    return RLoc(@"Sample foliage",
                @"Color panel button that samples a patch of grass or leaves to "
                @"measure how far off it is.");
  case MirageMemoryColorSky:
    return RLoc(@"Sample sky", @"Color panel button that samples a patch of clear "
...
static NSString *MirageSampleTooltip(MirageMemoryColor kind) {
  ...
  return RLoc(@"Click something in the frame that should be grey. The cross shows "
...
// The sampler is a PULL-DOWN, not a toggle: one button, four things the patch can
// be declared to be.
//
// A row of declarations, or a second button beside the sampler, was the obvious
// shape and the wrong one - the header had just been cleared of exactly that, and
// choosing what you are about to sample is the same gesture as arming the sampler,
// not a separate mode to set first.
...
//
// The pick is held as a POSITION and re-measured every frame, so leaving it in place
// would have the same pixels judged against a region they were never picked for -
// the cross would jump to a confident reading of a patch nobody declared.
...
// Arm the picker. The next click anywhere in the mini viewer samples that point.
//
// Caught with an event monitor rather than by hooking the mini viewer: the preview
// already owns its clicks for OSC handles, and a picker that fought that would break
// dragging. Monitors are also the only thing that reliably sees this process's mouse
// stream.
- (void)_armPicking {
  [self _disarmPicking];
  _picking = YES;
...
//
// One-shot and self-cancelling, like the other two - the click disarms, pressing
// the button again disarms, and Escape disarms - because an arming state you can
// forget you are in turns the next ordinary click on the preview into a parameter
...
- (void)_installPickMonitors {
  ...
  // process as a locally-dispatched event - the inspector's clicks are forwarded and
  // re-sent - so the local monitor alone saw nothing at all.
      ...
      addLocalMonitorForEventsMatchingMask:NSEventMaskLeftMouseDown
                                   handler:^NSEvent *(NSEvent *e) {
                                     ...
                                     if ([s _handlePickEvent:e])
                                       return nil; // consumed: don't also grab a handle
                                     return e;
                                   }];
  _pickGlobalMonitor =
      ...
      addLocalMonitorForEventsMatchingMask:NSEventMaskKeyDown
                                   handler:^NSEvent *(NSEvent *e) {
                                     ...
                                     if (s && e.keyCode == kEscapeKeyCode) {
                                       [s _disarmPicking];
                                       return nil; // consumed: don't also close the popover
                                     }
                                     return e;
                                   }];
...
/// Crosshair while the pointer is over the picture, the ordinary arrow elsewhere.
/// Set from the event stream rather than from a tracking area, because the mini
/// viewer's own cursor handling does not run for a panel in another window.
...
- (BOOL)_handlePickEvent:(NSEvent *)event {
  ...
  // Screen point from the CGEvent: a global monitor's event has no window, and a
  // forwarded one's window is not necessarily the one the cursor is over.
  NSPoint screen;
  ...
  NSPoint inView = [mini convertPoint:[mini.window convertPointFromScreen:screen]
                            fromView:nil];
  CGRect content = [mini contentRectInViewPoints];
  if (!NSPointInRect(inView, content))
    return NO; // clicked outside the image: leave the click alone and stay armed
  // Both the view and the processed texture are bottom-origin, so this maps across
  // without a flip.
  ...
  if (_pickingSource) {
    ...
    // the grade changes the pixel that was clicked, and re-picking the same spot
    // walks the value further every time. The clip's untouched footage is the
    // only fixed thing to measure against.
...
// from. The click's own `_sampleOnce` runs synchronously inside an NSEvent monitor,
// part-way through dispatching the very mouse-down that armed the picker, and the
// sampler reads the probe on that same call - so the common case wrote a parameter
// from inside event dispatch. The slow case runs off the mini viewer's
// frame-ready callback instead. FCP reported a caught exception from inside our own
// "Adjust Mirage" action - FFChannelChangeContext willSetChannel: through
// FFChannelAction lockChannels - which is what its channel lock raises when a
// parameter write re-enters it. Handing the write to the main queue costs a frame
// nobody can see and makes it an ordinary edit again.
- (void)_schedulePickWrite:(NSArray<NSNumber *> *)rgb
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    ...
    // the drag's mouse-up may have gone to another application - and opening the
    // pick's group inside that one is the nesting FCP's channel lock raises on.
...
//
// One undo group for the whole set, like a puck drag: the click was one gesture,
// and a shader mapping hue, saturation and a swatch would otherwise cost three
// steps back to undo one action.
- (void)_applyPickedRGB:(NSArray<NSNumber *> *)rgb
  ...
  NSDictionary<NSString *, NSNumber *> *picks =
      activePuckOnly ? [self _picksForActivePuckIn:timeline source:source]
                     : MirageSurfacePicksForSource(source);
  if (activePuckOnly)
    KKLogInfo(@"[Pick] click-to-pick reaches %lu of the shader's %lu pick= "
              @"control(s)",
              (unsigned long)picks.count,
              (unsigned long)MirageSurfacePicksForSource(source).count);
  if (!picks.count)
    return;
  ...
  // the helper takes. Saturation is Oklab chroma as a fraction of 0.33, the most
  // colourful thing Rec.709 can show, which is the convention the shader
  ...
  for (NSUInteger i = 0; i < lanes.count; i++) {
    ...
    case MirageSurfacePickKindLuma: {
      double next = boxed.integerValue == MirageSurfacePickKindLuma ? luma
                                                                    : saturation;
      // Both are measured 0..1 - saturation as Oklab chroma over 0.33 - and a
      // control declaring a max well above that is
      // holding percent. Read off the declared range rather than from the
      // directive kind, so a `#float max=100` behaves like the `#percent` it is.
  ...
  updated.lanes = lanes;
  // Logged on both sides on purpose. If the channel-lock exception comes back, the
  // question is whether one of these writes was still open when it was raised, and
  // a "beginning" with no "done" answers it - as does neither line, which would say
  // the picker had nothing to do with it.
...
/// YES when this shader has a `pick=` control the eyedropper could actually write.
/// Same visibility gate as the puck: a control a `visibleby=` rule has hidden is
/// not part of the gesture, and a picker that writes only invisible controls would
/// look broken.
- (BOOL)_hasDrivablePicksIn:(KKTimeline *)timeline source:(NSString *)source {
  NSDictionary<NSString *, NSNumber *> *picks = MirageSurfacePicksForSource(source);
  if (!picks.count)
    return NO;
...
/// control it is pointed at - "Pick Pivot", "Pick Target Hue" - because that is the
/// only honest label: the same button sets a luminance in one shader and a hue in
/// the next, so any fixed wording would be wrong somewhere. With several
/// subscribers there is no single name to borrow, so it falls back to the generic.
- (void)_refreshHeaderButtonTitlesIn:(KKTimeline *)timeline
  ...
  // measure rather than making the choice something you have to remember making.
  ...
  if (!_pickColorButton.hidden) {
    ...
    _pickColorButton.title =
        labels.count == 1
            ? [NSString stringWithFormat:RLoc(@"Pick %@",
                                              @"Color panel button that sets one "
                                              @"named control from a colour "
                                              @"clicked in the frame. %@ is that "
                                              @"control's name."),
                                         labels.firstObject]
            : RLoc(@"Pick from frame", @"Color panel button that sets several "
                                       @"controls at once from a colour clicked "
                                       @"in the frame.");
    _pickColorButton.toolTip =
        RLoc(@"Click a color in the frame to set this shader's controls from it.",
...
/// memory-colour wedge bind there: a picked colour is a hue, and a tonal ramp has
/// nowhere to put one. Otherwise the first declared circle, which is the only one
/// a single-surface shader has.
...
/// The `pick=` controls the ACTIVE puck owns, in the shape the write path takes.
///
/// `puck=` is the pairing, and it is the ONLY pairing. A subscriber declaring it
/// answers to that handle and no other, whether or not it also declares a
/// `surface=` - which is what lets a three-slot shader keep a target hue and the
/// centre it is measured from on the same handle even though only one of them is
/// draggable.
///
...
/// would fill every slot with the same colour, and the author would have no way to
/// tell it to stop. A shader that names no puck at all has one unnamed handle
/// every control belongs to, so nothing is narrowed there.
- (NSDictionary<NSString *, NSNumber *> *)
  ...
  NSString *puckName = [self _puckNameAtIndex:activePuck
                                         ring:ringIndex
                                       source:source]
                           ?: @"";
  NSDictionary<NSString *, NSString *> *puckNames =
      MirageSurfacePuckNamesForSource(source);
