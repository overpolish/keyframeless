...
// Shared Metal helpers used by every Shader generator Type: the reference-frame
// synthesis, the noise / hash primitives, and the core film-grain overlay.
// Extracted from Shader.metal so each Type's shader can rely on one place.
...
// (tiling, + 0.5, etc.) is applied by the caller. Shader (y-down), Dithering and
// Grainy build their own frames.
static float2 shader_objectUV(float2 tc, float aspect, constant ShaderCommonUniforms &cm) {
...
static float3 shader_applyGrain(float3 color, float2 fragCoord, constant ShaderCommonUniforms &cm) {
