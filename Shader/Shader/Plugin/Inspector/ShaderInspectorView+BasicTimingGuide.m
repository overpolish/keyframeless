...
static BOOL ShaderGuideOriginNearTarget(NSPoint p) {
  CGPoint t = ShaderGuideTargetObjectPosition();
  return hypot(p.x - t.x, p.y - t.y) < kShaderOSCGuideTargetSnap;
}
...
- (KKTimingGuideConfig *)_timingGuideConfig {
  ...
  cfg.viewerScreenRect = ^NSRect {
    return ShaderSharedOSCGuideBridge().estimatedViewerScreenRect;
  };
  cfg.oscGuideBridge = ^KKOSCGuideBridge * {
    return ShaderSharedOSCGuideBridge();
  };
  ...
  cfg.oscGuideStrategy = ^KKOSCGuideStrategy * {
    ...
    s.currentValue = ^id {
      return [NSValue valueWithPoint:ShaderGuideCurrentOrigin(weakLanes)];
    };
    s.setLiveValue = ^(id v) {
      NSPoint p = [v pointValue];
      ShaderSetGuidePosition(p.x, p.y); // OSC handle tracks the drag
    };
    s.valueForScreenPoint = ^id(NSPoint pt) {
      double x = 0.5, y = 0.5;
      ShaderGuidePositionForScreenPoint(pt, &x, &y);
    ...
    s.valueOnTarget = ^BOOL(id v) {
      return ShaderGuideOriginNearTarget([v pointValue]);
    };
    s.snapValue = ^id(id v) {
      NSPoint p = [v pointValue];
      if (ShaderGuideOriginNearTarget(p)) {
        CGPoint t = ShaderGuideTargetObjectPosition();
