> These instructions apply to v1. Run the commands below from `Legacy/`; use the [root guidance](../CONTRIBUTING.md) for current development.
...
- Shared logic lives in `KeyframelessKit`, not per-plugin. Reuse the kit base helpers (`KKPlugin`, `KKPointOSC`/OSC bases, `KKMiniViewerRenderer`, `KKPlugin+OSCVisibility`, the timeline-write and inspector-callback helpers) rather than re-deriving them.
