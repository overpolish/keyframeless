...
// Former Match Out row IDs remain hidden and ignored; legacy lanes kept their match state in the data blobs.
// Match In/Out is lane-wide, so the custom properties hold it in one hidden toggle each rather than in the keyframed pose payload.
...
enum { MMTransitionDuration = 1102, MMPositionX = 1300, MMDurationData = 1301, MMPositionAvailableTime = 1302, MMPositionLink = 1303, MMPositionMatch = 1304, MMPositionLegacyMatchOut = 1305, MMPositionEasing = 1306, MMPositionAddedMotion = 1307, MMPositionY = 1308,
       ...
       // On-screen control visibility: Position owns the box outline, Scale the handles.
       MMShowPositionOSC = 1703, MMShowScaleOSC = 1704 };
