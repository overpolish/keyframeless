...
#import "ShaderColorSpace.h"  // ShaderColorLabel, KK_SHADER_COLOR_COUNT_LABEL
#import "ShaderLaneCatalog.h" // ShaderBuildAvailableLanes
#import <KeyframelessKit/KKPresets.h>
#import <KeyframelessKit/KKTimingStage.h>

// One built-in look: a Type index plus a curated sRGB palette. Every other lane
// is left at its Shader default (unlisted lanes don't merge). Colours are 0..1
...
typedef struct {
  ...
  int type;               // Shader Type index (0..11)
  int colorCount;         // 2..cap for the Type
  float colors[KK_SHADER_COLOR_MAX][3];
} ShaderPresetSpec;

static const ShaderPresetSpec kShaderPresets[] = {
    // Shader, meadow green into deep night (#a7e58b #324472 #0a180d).
...
static KKLane *ShaderCatalogLane(NSArray<KKLane *> *catalog, NSString *label) {
...
static KKLane *ShaderPresetConstLane(NSArray<KKLane *> *catalog, NSString *label,
                                   NSArray<NSNumber *> *values) {
  KKLane *src = ShaderCatalogLane(catalog, label);
  if (!src)
    return nil;
...
NSArray<KKPreset *> *ShaderBuiltinPresets(void) {
  NSArray<KKLane *> *catalog = ShaderBuildAvailableLanes();
  NSMutableArray<KKPreset *> *out = [NSMutableArray array];
  const size_t count = sizeof(kShaderPresets) / sizeof(kShaderPresets[0]);
  for (size_t i = 0; i < count; i++) {
    const ShaderPresetSpec *s = &kShaderPresets[i];
    NSMutableArray<KKLane *> *lanes = [NSMutableArray array];
    KKLane *typeLane = ShaderPresetConstLane(catalog, @"Type", @[ @(s->type) ]);
    if (typeLane)
      [lanes addObject:typeLane];
    KKLane *countLane = ShaderPresetConstLane(catalog, KK_SHADER_COLOR_COUNT_LABEL,
                                            @[ @(s->colorCount) ]);
    if (countLane)
      [lanes addObject:countLane];
    for (int c = 0; c < s->colorCount && c < KK_SHADER_COLOR_MAX; c++) {
      NSArray<NSNumber *> *rgba =
          @[ @(s->colors[c][0]), @(s->colors[c][1]), @(s->colors[c][2]), @1.0 ];
      KKLane *colorLane = ShaderPresetConstLane(catalog, ShaderColorLabel(c), rgba);
      if (colorLane)
