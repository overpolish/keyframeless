...
/// The bundle that carries Mesh's compiled string catalog
/// (MeshUI.xcstrings). Look strings up against this bundle - never
...
FOUNDATION_EXPORT NSBundle *MeshLocalizationBundle(void);

NS_ASSUME_NONNULL_END

/// Localize a user-visible Mesh string from the MeshUI table, resolved
...
#define RLoc(key, comment)                                                     \
  NSLocalizedStringFromTableInBundle((key), @"MeshUI",                      \
                                     MeshLocalizationBundle(), (comment))
