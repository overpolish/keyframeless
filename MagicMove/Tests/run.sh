...
for suite in ${MM_TEST_SUITES:-HostLifecycleTests HeaderTests LinkedPosesTests ModelTests MatchEndpointsTests CombinedPoseTests EasingTests AddedMotionTests MotionBlurTests ShortcutTests CustomRowTests PositionTests ScaleTests TimingEditorTests OpacityTests RotationTests BlurAnchorTests ResetParameterTests NativeLinksTests}; do
  xcrun clang -fobjc-arc -fmodules -Wno-protocol -fsanitize=address,undefined \
    ...
    "$core/MMParameterData.m" "$core/Plugin.m" "$core/Plugin+CustomRow.m" "$core/Plugin+Links.m" "$core/Plugin+Parameters.m" \
