...
@end
// A changed pose per lane: every component moved, with non-default timing.
static id changedPose(UInt32 p) {
  KFPropertyLane *lane=KFPropertyLaneForParameter(p);
  ...
  for(NSUInteger axis=0;axis<lane.componentCount;axis++)
    [values addObject:@(lane.defaultPose.values[axis].doubleValue+30+axis*15)];
  return [lane.defaultPose poseByReplacingValues:values authored:YES easing:MTEasingLinear
      addedMotion:MTAddedMotionWave timing:[KFPoseTiming new]];
}
static void seed(ResetHost *host, UInt32 p) {
  id pose=[changedPose(p) poseByReplacingTiming:[[KFPoseTiming alloc] initWithDuration:3 available:YES amount:2 speed:4]]; host.blobs[@(p)]=pose;
  for(int t=0;t<3;t+=2) {
...
static void refreshSnapshots(ResetHost *host) {
  ...
  for(KFPropertyLane *lane in KFPropertyLanes()) [lane refreshCacheForManager:host time:TestTime(1)];
  [host endAction:host];
...
static void checkDefault(id pose, UInt32 p) {
  ...
  KFPoseTiming *timing=[pose timing]; assert(!timing.available && timing.duration==1.2 && timing.amount==1 && timing.speed==1);
  KFPropertyLane *lane=KFPropertyLaneForParameter(p);
  assert([[pose values] isEqualToArray:lane.defaultPose.values]);
}
@interface KFTestEffect (ResetTest)
- (NSView *)createViewForParameterID:(UInt32)p NS_RETURNS_RETAINED;
@end
int main(void) { @autoreleasepool {
  KFTestRegisterLanes();
  [NSApplication sharedApplication];
  NSArray *parameters=@[@(KFTestPosition),@(KFTestScale),@(KFTestOpacity),@(KFTestRotation)];
  for(NSNumber *number in parameters) {
    ...
    KFPropertyPoseCache *position=[KFTestPositionLane() createCache]; host.staticValues[@(KFTestPositionCacheToken)]=position.token;
    KFPropertyPoseCache *scale=[KFTestScaleLane() createCache]; host.staticValues[@(KFTestScaleCacheToken)]=scale.token;
    KFPropertyPoseCache *opacity=[KFTestOpacityLane() createCache]; host.staticValues[@(KFTestOpacityCacheToken)]=opacity.token;
    KFPropertyPoseCache *rotation=[KFTestRotationLane() createCache]; host.staticValues[@(KFTestRotationCacheToken)]=rotation.token;
    refreshSnapshots(host);
    assert(KFResetParameter(host,[NSView new],p));
    assert(host.keyInfoReads==0 && host.refreshWrites==1);
    NSString *firstToken=host.blobs[@(KFTestHostRefreshToken)];
    ...
    NSArray *entries=p==KFTestPosition ? position.snapshotEntries:p==KFTestScale ? scale.snapshotEntries:p==KFTestOpacity ? opacity.snapshotEntries:rotation.snapshotEntries;
    assert(entries.count==1 && !entries[0][@"nativeTime"]); checkDefault(entries[0][@"pose"],p);
    assert(KFResetParameter(host,[NSView new],p)); assert(![host lane:p].count); // repeated constant reset
    assert(host.refreshWrites==2 && ![firstToken isEqual:host.blobs[@(KFTestHostRefreshToken)]]);
    ...
    for(int failure=0;failure<4;failure++) {
      ...
      assert(!KFResetParameter(failed,[NSView new],p));
    ...
    KFTestEffect *plugin=[[KFTestEffect alloc] initWithAPIManager:rowHost]; rowHost.plugin=plugin;
    ...
    BOOL hasOSC=p==KFTestPosition || p==KFTestScale || p==KFTestRotation || p==KFTestAnchor;
