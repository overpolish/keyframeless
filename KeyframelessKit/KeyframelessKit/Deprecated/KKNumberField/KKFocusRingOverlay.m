...
//   _glowLayer.strokeColor =
//       [_color colorWithAlphaComponent:KKNumberFieldGlowAlpha].CGColor;
//   _glowLayer.lineWidth = KKNumberFieldGlowLineWidth;
//   [_containerLayer addSublayer:_glowLayer];

//   // Ring is a filled even-odd compound path (outer rounded rect + inner
//   hole).
//   // This ensures the stroke can never extend inward regardless of animation
//   // state.
...
// /// Compound fill path for the ring: outer rounded rect minus inner rounded
// rect
// /// hole. |expansion| expands the outer boundary outward beyond its
// steady-state
// /// position. The inner boundary is always fixed so the ring never encroaches
// /// inward.
...
//                          w - 2 * p - KKNumberFieldPrefixWidth -
//                              KKNumberFieldSuffixWidth,
//                          h - 2 * p);
//   NSRect focusRect =
//       NSInsetRect(fr, KKNumberFieldFocusInset, KKNumberFieldFocusInset);
//   CGFloat halfLW = KKNumberFieldFocusLineWidth / 2.0;

//   NSRect outerRect =
//       NSInsetRect(focusRect, -(halfLW + expansion), -(halfLW + expansion));
...
//   CGPathAddRoundedRect(path, NULL, NSRectToCGRect(outerRect), outerRadius,
//                        outerRadius);
//   CGPathAddRoundedRect(path, NULL, NSRectToCGRect(innerRect), innerRadius,
//                        innerRadius);
//   return path;
// }
...
//   // The panel frame is padded on all sides by _padding so the ring stroke is
//   // never clipped at the window edge. Compute the field rect within those
//   // padded bounds.
//   CGFloat p = _padding;
//   NSRect fr = NSMakeRect(p + KKNumberFieldPrefixWidth, p,
//                          w - 2 * p - KKNumberFieldPrefixWidth -
...
//   // Position the glow so its outer edge aligns with the ring's inner
//   boundary.
//   // The glow then extends inward from there — the ring fill covers the
//   outward
//   // half.
//   CGFloat halfLW = KKNumberFieldFocusLineWidth / 2.0;
...
//   // Set final model state first so layers snap to end values when animations
//   // finish.
//   [CATransaction begin];
//   [CATransaction setDisableActions:YES];
...
//   CABasicAnimation *pathAnim = [CABasicAnimation
//   animationWithKeyPath:@"path"]; pathAnim.fromValue = (__bridge id)startPath;
...
//   // Custom aggressive ease-in — stays near-transparent before snapping to
//   full
//   // opacity.
//   CAMediaTimingFunction *aggressiveEaseIn =
//       [CAMediaTimingFunction functionWithControlPoints:0.9:0.0:1.0:1.0];
//   CABasicAnimation *fade = [CABasicAnimation
//   animationWithKeyPath:@"opacity"]; fade.fromValue = @0.0; fade.duration =
//   duration; fade.timingFunction = aggressiveEaseIn;
