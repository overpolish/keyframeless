...
for suite in ${MM_TEST_SUITES:-PreferenceTests HostLifecycleTests HeaderTests ModelTests LaneIntegrationTests MotionBlurTests ShortcutTests OSCWriteTests}; do
  xcrun clang -fobjc-arc -fmodules -Wno-protocol -fsanitize=address,undefined \
    ...
    "$core/MMInspectorHeader.m" "$inspector/ICInspectorHeader.m" "$inspector/ICPopUpButton.m" "$inspector/ICMenuToggleView.m" "$inspector/InspectorTokens.m" "$inspector/ICValueTextField.m" "$inspector/ICValueFieldEditor.m" "$inspector/ICValueTextField+Scrub.m" "$inspector/ICValueFieldNavigation.m" "$inspector/ICInspectorRow.m" "$inspector/ICSliderView.m" "$inspector/ICSliderRow.m" "$core/MMLanes.m" "$core/MMShortcut.m" "$render/Plugin+Render.m" "$render/Plugin+RenderGeometry.m" "$osc/MagicMoveOSC.m" "$viewer/OSCCursor.m" "$viewer/OSCViewerControl.m" "$viewer/OSCViewerControl+Draw.m" "$viewer/OSCViewerControl+Input.m" "$viewer/OSCViewerControl+Rotation.m" "$viewer/OSCViewerControl+Anchor.m" "$support/RenderSupport.m" "$support/RSMetalResources.m" "$support/RSOSCDrawing.m" \
