...
static inline NSArray<KKLane *> *ShaderBuildAvailableLanes(void) {
  ...
  // --- Shader controls (Type 0). %-units store 0..100 (the shader wants 0..1).
  struct {
    NSString *label;
    double def, min, max;
  ...
  // are the lanes shared with Shader / Grainy above.
  struct {
    NSString *label;
    double def, min, max;
  ...
  for (int i = 0; i < KK_SHADER_COLOR_MAX; i++) {
    int n = i + 1; // 1-based swatch number
    KKLane *color = [KKLane laneWithLabel:ShaderColorLabel(i)];
    ...
    color.visibleWhenValues = ShaderTypesWithColorCap(n);
    color.visibleWhenAndLabel = KK_SHADER_COLOR_COUNT_LABEL;
    color.visibleWhenAndValues = ShaderColorCountAtLeast(n);
