...
@implementation ShaderPlugin (RenderMultipass)
...
- (ShaderSimRange)simRangeForSet:(ShaderFeedbackSet *)fb
  ...
  if (F < 0) // unknown frame index: plain one-step advance (non-deterministic)
    return (ShaderSimRange){0, 0, NO};
  if (fb->hasState && F == fb->lastFrame)
    return (ShaderSimRange){1, 0, NO}; // reuse existing state
  if (fb->hasState && F == fb->lastFrame + 1)
    return (ShaderSimRange){F, F, NO}; // sequential: one step forward
  NSInteger cp = [fb nearestCheckpointAtMost:F];
  if (cp < 0) // cold seek: re-sim a bounded window from clear
    return (ShaderSimRange){MAX((NSInteger)0, F - kFeedbackWindow), F, YES};
  [fb restoreFrame:cp device:device queue:queue];
  fb->lastFrame = cp;
  fb->hasState = YES;
  return (ShaderSimRange){cp + 1, F, NO};
}
...
static NSArray *ShaderChannelsForBuffer(int k, ShaderFeedbackSet *fb, int curI,
...
- (BOOL)renderCustomMultipassWithUniforms:(KKGLSLUniforms)u
  ...
  ShaderFeedbackSet *fb = [ShaderFeedbackSet setInStore:self.feedbackSets
                                                 forKey:fbKey
                                                  width:bufW
                                                 height:bufH];
  ...
  ShaderSimRange sim = [self simRangeForSet:fb
                                 frameIndex:F
                                     device:device
                                      queue:ckptQueue];
  ...
  for (NSInteger f = sim.from; f <= sim.to; f++) {
    ...
    for (int k = 0; k < 4; k++) {
      ...
      NSArray *chArr = ShaderChannelsForBuffer(k, fb, curI, prevI, present,
                                               noPrev, srcTex, &bufU);
