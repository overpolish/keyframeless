...
// Cap the pixels actually inspected. The preview is small, but it is sized to the
// popover's display resolution, so this keeps the cost flat if that grows.
static const NSUInteger kMaxSamples = 40000;
/// Oklab a/b that puts the cast marker at the rim. A cast is a small residual, so it
/// needs its own much tighter scale than the cloud to be visible at all. Sized from
/// measurement: a cast strong enough to be obviously wrong reads about 0.055.
static const double kCastFullScale = 0.08;

@implementation MirageScopeReading
@end

/// Half-width of the sampled patch, in pixels. A single pixel would be at the mercy
/// of sensor noise and compression blocks, so a small neighbourhood is averaged.
...
/// Which of the byte layouts this file understands `texture` is in, or NO for one
/// it does not. A format this does not understand must be refused rather than
/// reinterpreted, which would report a confidently wrong reading.
...
/// Average the patch around `uv`, returning NO when the point is outside the frame
/// or the patch lands on no pixels at all. `outLinear` and `outCoded` each take
/// three components and either may be NULL: the two picks want different encodings
/// of the same pixels, and reading the neighbourhood twice to serve them would be
/// the same loop with one line changed.
...
/// rather than from `wantScoped`. A half-scoped state - one set allocated, or the
/// flag saying yes while an allocation said no - is then not representable, which
/// is what keeps every later reader honest without re-checking.
...
/// Walk the readback on `stride` in both axes, accumulating every visited pixel in
/// linear light. u/v are bottom-origin, like the texture and like
...
/// Where the cast marker sits for a patch averaged in linear light: its Oklab a/b
/// error against `declaration`, scaled to the rim and clamped to the circle.
static NSPoint MirageScopeCastMarker(const double linear[3],
  ...
  // against the region that colour belongs in, and the error is what is left over
  // once the nearest correct version of it is subtracted - so a patch already in
  // the region reports exactly nothing.
  //
...
/// Blit `texture` into the shared-storage copy and hand back its bytes, or nil for
/// a texture this file cannot read. `bytesPerRow` is always `width * 4`.
...
- (MirageScopeReading *)readTexture:(id<MTLTexture>)texture
  ...
  // Sanitised here, not trusted from the property: a caller can set it before the
  // preview has been laid out, and a rect that is not a region has to read as the
  // whole frame rather than as a very small zoom.
  NSRect region = MirageScopeSanitisedVisibleRect(self.visibleUVRect);
  ...
  // ONE pass. The pixel that lands in the visible rect increments its bin in both
  // sets as it is read, so the zoomed scope costs the same single readback the
  // unzoomed one does.
  ...
  // A zoom so tight that the stride steps over the whole region leaves nothing to
  // draw. The full-frame set is still honest, so the reading falls back to it
  // rather than showing an empty bright layer over a ghost.
  ...
  // The picked reference: average a small patch so noise and compression blocking
  // do not decide the reading.
  double picked[3] = {0.0, 0.0, 0.0};
  if (MirageAveragePatch(bytes, w, h, bytesPerRow, rIdx, bIdx, self.pickUV,
                         picked, NULL)) {
    reading.chromaCast = MirageScopeCastMarker(picked, self.pickDeclaration);
    ...
    // pixels it has nothing to do with, and one circle would be saying two things
    // at once. Withdrawing it is the honest answer - the reference has scrolled
    // out of the picture - and castAvailable already means exactly that.
    reading.castAvailable =
        !scoped || MirageScopeUVInRect(self.pickUV, region);
  }

  double probed[3] = {0.0, 0.0, 0.0};
  if (MirageAveragePatch(bytes, w, h, bytesPerRow, rIdx, bIdx, self.probeUV,
                         NULL, probed))
