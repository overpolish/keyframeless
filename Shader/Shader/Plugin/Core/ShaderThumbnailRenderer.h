...
/// describes (its timeline / values) into a `w`x`h` PNG, sampling a synthetic
/// test-pattern source on iChannel0 so texture-processing shaders have detail.
/// Returns nil if Metal is unavailable. Used to bake `preview.png` on save +
/// publish, so a catalog entry shows what the shader looks like.
FOUNDATION_EXPORT NSData *_Nullable ShaderRenderThumbnailPNG(
