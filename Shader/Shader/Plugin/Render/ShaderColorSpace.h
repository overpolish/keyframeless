...
// Shader Gradient takes a flat list of colour swatches (no positions - the shader
...
/// add/remove swatches (2..KK_SHADER_COLOR_MAX) via the Colors section; the live
/// count is stored in the "Color Count" lane.
#define KK_SHADER_COLOR_COUNT 4

/// Absolute maximum number of swatch lanes ("Color 1".."Color 10"). Matches the
/// widest shader palette array (KK_SHADER_GRAD_COLORS). Per-Type caps below may
/// be lower (see ShaderMaxColorsForType).
#define KK_SHADER_COLOR_MAX KK_SHADER_GRAD_COLORS
...
static const float kShaderDefaultColorsSRGB[KK_SHADER_COLOR_MAX][4] = {
...
static inline int ShaderMaxColorsForType(int type) {
  switch (type) {
  case ShaderType_GrainGradient:
    return KK_GRAIN_GRAD_COLORS; // 7
  case ShaderType_GodRays:
    return 5;
  case ShaderType_Custom:
    return 0; // the user shader owns its own colours (no palette lanes yet)
  default:
    return KK_SHADER_GRAD_COLORS; // 10
  }
}

/// Shader Gradient (paper-design port) scalar defaults.
#define KK_SHADER_GRAD_DEFAULT_DISTORTION 0.80f
#define KK_SHADER_GRAD_DEFAULT_SWIRL 0.10f
#define KK_SHADER_GRAD_DEFAULT_SPEED 1.0f // time multiplier (1 = source rate)
#define KK_SHADER_GRAD_DEFAULT_SEED 0.0f

/// Core film-grain overlay, shared by every Type (ShaderCommonUniforms). A subtle
...
static inline float ShaderGrainScaleForType(int type) {
  switch (type) {
  case ShaderType_GrainGradient:
...
/// Fallback Shader Gradient uniforms (the default palette as spots) so an
/// un-edited instance still renders before the colour lanes resolve.
static inline ShaderGradientUniforms ShaderGradientDefault(void) {
  ShaderGradientUniforms g;
  memset(&g, 0, sizeof(g));
  g.colorsCount = KK_SHADER_COLOR_COUNT;
  for (int i = 0; i < KK_SHADER_COLOR_COUNT; i++) {
    const float *c = kShaderDefaultColorsSRGB[i];
    g.colors[i] = (vector_float4){c[0], c[1], c[2], c[3]};
  }
  g.distortion = KK_SHADER_GRAD_DEFAULT_DISTORTION;
  g.swirl = KK_SHADER_GRAD_DEFAULT_SWIRL;
  return g;
}
...
static inline DitheringUniforms DitheringDefault(void) {
  DitheringUniforms d;
  memset(&d, 0, sizeof(d));
  d.colorsCount = KK_SHADER_COLOR_COUNT;
  for (int i = 0; i < KK_SHADER_COLOR_COUNT; i++) {
    const float *c = kShaderDefaultColorsSRGB[i];
...
/// before the lanes resolve. Colours share the Shader default palette (the same
...
static inline GrainGradientUniforms GrainGradientDefault(void) {
  ...
  memset(&g, 0, sizeof(g));
  int n = KK_SHADER_COLOR_COUNT < KK_GRAIN_GRAD_COLORS ? KK_SHADER_COLOR_COUNT
...
/// shared with Shader/Grainy, so their fallbacks match those shared defaults.
...
/// lanes resolve. Colours share the Shader default palette. `resolution` is
/// overwritten at render time.
static inline WarpUniforms WarpDefault(void) {
  WarpUniforms w;
  memset(&w, 0, sizeof(w));
  w.colorsCount = KK_SHADER_COLOR_COUNT;
  for (int i = 0; i < KK_SHADER_COLOR_COUNT; i++) {
    const float *c = kShaderDefaultColorsSRGB[i];
  ...
  w.distortion = KK_SHADER_GRAD_DEFAULT_DISTORTION; // shared "Distortion" lane
  w.swirl = KK_SHADER_GRAD_DEFAULT_SWIRL;           // shared "Swirl" lane
...
static inline NeuroNoiseUniforms NeuroNoiseDefault(void) {
  ...
  memset(&n, 0, sizeof(n));
  n.colorsCount = KK_SHADER_COLOR_COUNT;
  for (int i = 0; i < KK_SHADER_COLOR_COUNT; i++) {
    const float *c = kShaderDefaultColorsSRGB[i];
...
static inline SimplexNoiseUniforms SimplexNoiseDefault(void) {
  ...
  memset(&s, 0, sizeof(s));
  s.colorsCount = KK_SHADER_COLOR_COUNT;
  for (int i = 0; i < KK_SHADER_COLOR_COUNT; i++) {
    const float *c = kShaderDefaultColorsSRGB[i];
...
static inline MetaballsUniforms MetaballsDefault(void) {
  MetaballsUniforms m;
  memset(&m, 0, sizeof(m));
  m.colorsCount = KK_SHADER_COLOR_COUNT;
  for (int i = 0; i < KK_SHADER_COLOR_COUNT; i++) {
    const float *c = kShaderDefaultColorsSRGB[i];
...
static inline GodRaysUniforms GodRaysDefault(void) {
  GodRaysUniforms g;
  memset(&g, 0, sizeof(g));
  int rays = KK_SHADER_COLOR_COUNT < 5 ? KK_SHADER_COLOR_COUNT : 5;
  g.colorsCount = rays;
  for (int i = 0; i < KK_SHADER_COLOR_COUNT; i++) {
    const float *c = kShaderDefaultColorsSRGB[i];
    g.colors[i] = (vector_float4){c[0], c[1], c[2], c[3]};
...
static inline FluidUniforms FluidDefault(void) {
  FluidUniforms f;
  memset(&f, 0, sizeof(f));
  f.colorsCount = KK_SHADER_COLOR_COUNT;
  for (int i = 0; i < KK_SHADER_COLOR_COUNT; i++) {
    const float *c = kShaderDefaultColorsSRGB[i];
...
static inline NSString *ShaderColorLabel(int i) {
  return [NSString stringWithFormat:@"Color %d", i + 1];
}

/// Label of the swatch-count lane: component-0 holds the active swatch count
/// (2..KK_SHADER_COLOR_MAX). Drives both how many "Color N" rows show and how
/// many swatches the render reads (clamped per-Type by ShaderMaxColorsForType).
#define KK_SHADER_COLOR_COUNT_LABEL @"Color Count"
...
static inline int ShaderEffectiveColorCount(int storedCount, int type) {
  int n = storedCount > 0 ? storedCount : KK_SHADER_COLOR_COUNT;
  int cap = ShaderMaxColorsForType(type);
  if (n > cap)
    n = cap;
  if (n > KK_SHADER_COLOR_MAX)
    n = KK_SHADER_COLOR_MAX;
  return n < 0 ? 0 : n;
}

/// Number of Types (choice-pill entries). Keep in sync with the ShaderType enum.
#define SHADER_TYPE_COUNT 13
...
static inline NSArray<NSNumber *> *ShaderTypesWithColorCap(int minCap) {
  NSMutableArray<NSNumber *> *out = [NSMutableArray array];
  for (int t = 0; t < SHADER_TYPE_COUNT; t++)
    if (ShaderMaxColorsForType(t) >= minCap)
      [out addObject:@(t)];
  return out;
}
...
static inline NSArray<NSNumber *> *ShaderColorCountAtLeast(int n) {
  NSMutableArray<NSNumber *> *out = [NSMutableArray array];
  for (int c = n; c <= KK_SHADER_COLOR_MAX; c++)
    [out addObject:@(c)];
  return out;
}
/// 0-based index if `label` is "<prefix> N", else -1.
static inline int ShaderIndexForLabel(NSString *label, NSString *prefix) {
