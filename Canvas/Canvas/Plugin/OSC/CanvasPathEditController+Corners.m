...
// The corner-radius widget under a surface point, or -1. Widgets sit just inside
// each interior corner (offset along the bisector), so they live in the path's
// empty area - checked after anchors / handles, before a marquee.
...
// mouseUp). The grabbed corner is `_grabCorner`, geometry frozen at `_dragStartGeom`.
- (void)_dragCornerToX:(double)x y:(double)y modifiers:(CanvasPenModifiers)mods {
  ...
  CanvasCornerWidget w =
      CanvasCornerWidgetObj(layers, start, frac, aspect, (NSUInteger)_grabCorner);
  if (!w.valid)
    return;
  _didEdit = YES;
  // Cursor in object space (post-layer-transform), projected onto the corner's
  // interior bisector; the offset matches the widget's resting gap, so there's
  // no jump on grab. Convert the object-space radius back to the stored local.
  ...
  [edited setCornerRadius:rObj * w.localPerObjScale
                  atIndex:(NSUInteger)_grabCorner];
