...
// (FxModifierKeys for the viewer, NSEventModifierFlags for the mini) into these.
typedef NS_OPTIONS(NSUInteger, CanvasPenModifiers) {
  CanvasPenModNone = 0,
  CanvasPenModShift = 1 << 0, // axis-lock the handle
...
/// OSC or the inspector mini-viewer. It abstracts everything that differs between
/// the two - coordinate conversion, grid snap, layer-blob I/O, and the overlay
/// drawing primitives. ALL object points crossing this protocol are NORMALIZED
/// and Y-UP (the KKBezierPath point space); the surface owns any flip to its own
/// space. "Surface points" are the surface's own input space (the viewer's
/// ioSurface px / the mini's view points).
@protocol CanvasPenSurface <NSObject>
- (CGPoint)penObjFromSurfaceX:(double)x y:(double)y;        // raw, no snap
- (CGPoint)penSnappedObjFromSurfaceX:(double)x y:(double)y; // grid-snapped (or raw)
- (CGPoint)penSurfacePointFromObj:(CGPoint)objYUp;
- (BOOL)penGridSnapping;
- (double)penCanvasAspect; // outputW/outputH, for 45deg handle snap in pixel space
- (BOOL)penToolActive;
...
/// bezier handles, closing / finishing, snap ghost, single-undo commits, and the
/// overlay preview. Shared by the viewer OSC and the mini-viewer so both behave
/// identically and new drawing features land once.
