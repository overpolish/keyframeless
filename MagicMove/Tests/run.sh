...
for suite in ${MM_TEST_SUITES:-LinkedPosesTests ModelTests MatchEndpointsTests CombinedPoseTests EasingTests AddedMotionTests MotionBlurTests ShortcutTests CustomRowTests PositionTests ScaleTests TimingEditorTests OpacityTests RotationTests}; do
  xcrun clang -fobjc-arc -fmodules -Wno-protocol -fsanitize=address,undefined \
    ...
    -framework ApplicationServices -framework IOSurface -framework CoreVideo -framework AppKit -framework Foundation -framework CoreGraphics -framework CoreMedia -framework Metal -framework FxPlug -framework KeyframelessKit \
    ...
    "$inspector/InspectorTokens.m" "$inspector/ICValueTextField.m" "$inspector/ICInspectorRow.m" "$inspector/ICSliderView.m" "$inspector/ICSliderRow.m" "$core/MMPoseTiming.m" "$core/MMTimingEditorModel.m" "$core/MMTimingEditor.m" "$core/MMScalePose.m" "$core/MMScalarPose.m" "$core/MMPropertyLane.m" "$core/MMRotationPose.m" "$core/MMPropertyRow.m" "$core/MMShortcut.m" "$core/MMCombinedPose.m" "$core/MMDestinations.m" "$render/Plugin+Render.m" \
