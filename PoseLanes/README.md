...
- `KFKeyposeMap.m` is the navigator under that graph: one stop per keypose of the selected lane on an ordinal axis, the hold/transition split drawn to scale inside each segment, and the geometry the whole view is addressed through. `KFKeyposeMap+Interaction.m` adds the mouse handling: a press on a keypose selects it and a drag retimes it between its neighbours, a press on the rail drags the playhead, and a right-click asks for the menu of the keypose under the pointer. The timing editor supplies the stops and every host time decision, so the view holds no host knowledge.
- `KFKeyposeEdits.m` adds, deletes and retimes one keypose at a time for that map, taking linked partners with a move and re-pairing a matched property afterwards. `KFTimingEditor+Keyposes.m` is the editor side of the map: the ordinal scrub, the frame grid a retime or an insertion snaps to, and the menu that adds a keypose where the pointer is or deletes the one under it.
