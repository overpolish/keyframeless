...
// The visibility toggles and the refresh token belong to a plugin; these stand
// in for them.
enum { TestShowPositionOSC = 1050, TestShowAnchorOSC = 1053, TestHostRefreshToken = 1060 };
...
- (void)startAction:(id)target { self.actions++; }
- (void)endAction:(id)target {}
- (BOOL)setCustomParameterValue:(id)value toParameter:(UInt32)parameter atTime:(CMTime)time {
  if (parameter == TestHostRefreshToken) self.refreshWrites++;
...
static NudgeHost *Host(void) {
  ...
  host.editors[@(TestShowPositionOSC)] = @YES;
  return host;
}
...
static void oneNudgePerStop(void) {
  NudgeHost *host = Host();
  KFOSCPlayheadNudge *nudge = [[KFOSCPlayheadNudge alloc] initWithManager:host
                                 visibilityParameters:@[@(TestShowPositionOSC), @(TestShowAnchorOSC)]];
...
static void noVisibleControlNoWrite(void) {
  NudgeHost *host = Host();
  host.editors[@(TestShowPositionOSC)] = @NO;
  KFOSCPlayheadNudge *nudge = [[KFOSCPlayheadNudge alloc] initWithManager:host
                                 visibilityParameters:@[@(TestShowPositionOSC), @(TestShowAnchorOSC)]];
  host.playhead = TestTime(0);
  ...
  host.editors[@(TestShowAnchorOSC)] = @YES;
  host.playhead = TestTime(2);
