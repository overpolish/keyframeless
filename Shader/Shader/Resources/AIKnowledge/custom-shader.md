...
- **Output is forced opaque.** Shadertoy ignores `fragColor.a`; so does this. A shader can't punch a hole in the clip by writing a low alpha.
...
- Sampling an unused `iChannel1-3` gives smooth value noise - good for grain, fbm, hashing (unless you've added a Buffer that routes to that channel).
