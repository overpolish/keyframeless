...
id<MTLTexture> ShaderNewBufferTexture(id<MTLDevice> device, NSUInteger w,
                                      NSUInteger h) {
...
@implementation ShaderFeedbackSet

+ (instancetype)setInStore:(NSMutableDictionary *)store
                    forKey:(NSString *)key
                     width:(NSUInteger)w
                    height:(NSUInteger)h {
  ShaderFeedbackSet *fb = store[key];
  if (!fb) {
    fb = [ShaderFeedbackSet new];
    store[key] = fb;
  }
  if (fb->w != w || fb->h != h) {
    for (int s = 0; s < 2; s++)
...
- (void)snapshotFrame:(NSInteger)frame
  ...
  for (int k = 0; k < 4; k++) {
    ...
    id<MTLTexture> copy = ShaderNewBufferTexture(device, w, h);
    if (copy)
...
- (void)restoreFrame:(NSInteger)frame
  ...
  for (int k = 0; k < 4; k++) {
    ...
    if (!dst) {
      dst = ShaderNewBufferTexture(device, w, h);
      tex[into][k] = dst;
    }
    if (dst)
