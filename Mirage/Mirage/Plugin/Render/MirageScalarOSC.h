...
// off the directive. Pure functions of a parsed MirageScalarProp.
#pragma once
...
static inline BOOL MirageScalarRingEligible(const MirageScalarProp *p) {
...
static inline BOOL MirageScalarRadialOSC(const MirageScalarProp *p) {
  return strcmp(p->oscKind, "ring") == 0 || strcmp(p->oscKind, "box") == 0;
}
static inline BOOL MirageScalarOSCIsBox(const MirageScalarProp *p) {
...
static inline BOOL MirageScalarOSCIsRotate(const MirageScalarProp *p) {
...
static inline int MirageScalarRotateArity(const MirageScalarProp *p) {
...
static inline int MirageScalarRotationAxisMask(const MirageScalarProp *p) {
  int mask = 0;
...
MirageRotateCanonicalSwizzle(const MirageScalarProp *p) {
...
static inline void MirageScalarParseOSC(NSString *attrs, MirageScalarProp *p) {
  ...
  if ([[NSRegularExpression regularExpressionWithPattern:@"\\bosc\\b"
    ...
    if (MirageScalarOSCIsRotate(p) && p->oscAxisCount == 0) {
    ...
    if (MirageScalarRadialOSC(p) || MirageScalarOSCIsRotate(p)) {
