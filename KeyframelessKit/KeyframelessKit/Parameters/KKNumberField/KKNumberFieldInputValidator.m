...
/// Returns YES for strings that are valid in-progress but not yet complete
/// numbers (e.g. "-", ".", "-.").
static BOOL KKIsPartialInput(NSString *string) {
  return [string isEqualToString:@"-"] || [string isEqualToString:@"."] ||
...
- (BOOL)isValidReplacementString:(NSString *)replacementString
  ...
  NSString *proposed =
      [currentString stringByReplacingCharactersInRange:affectedRange
  ...
  NSCharacterSet *input =
      [NSCharacterSet characterSetWithCharactersInString:replacementString];
  ...
  NSUInteger minusCount =
      [[proposed componentsSeparatedByString:@"-"] count] - 1;
  if (minusCount > 1)
    return NO;
