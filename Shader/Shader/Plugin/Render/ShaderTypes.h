...
typedef struct ShaderGradientUniforms {
    vector_float4 colors[KK_SHADER_GRAD_COLORS]; // rgba (straight alpha)
...
typedef struct DitheringUniforms {
    vector_float4 colors[KK_SHADER_GRAD_COLORS]; // rgba palette ramp (straight alpha)
    int colorsCount;                           // active colours (<= 10)
...
typedef struct WarpUniforms {
    vector_float4 colors[KK_SHADER_GRAD_COLORS]; // rgba (straight alpha), up to 10
    int colorsCount;                           // active colours (<= 10)
...
typedef struct MetaballsUniforms {
    vector_float4 colors[KK_SHADER_GRAD_COLORS]; // rgba (straight alpha), indexed modulo colorsCount
...
typedef struct GodRaysUniforms {
    vector_float4 colors[KK_SHADER_GRAD_COLORS]; // rgba (straight alpha); up to 5 used
...
typedef struct FluidUniforms {
    vector_float4 colors[KK_SHADER_GRAD_COLORS]; // rgba (straight alpha), up to 10 (~4 used)
    int colorsCount;                           // active colours (<= 10)
...
typedef struct NeonUniforms {
    vector_float4 colors[KK_SHADER_GRAD_COLORS]; // rgba (glow, surface, inner, core = 1..4)
...
typedef struct SilkUniforms {
    vector_float4 colors[KK_SHADER_GRAD_COLORS]; // rgba; Color 1..3 = the 3 layer hues
...
typedef struct StrataUniforms {
    vector_float4 colors[KK_SHADER_GRAD_COLORS]; // rgba; layers pick from these by hash
    int colorsCount;                           // active colours (<= 10)
...
typedef enum ShaderType {
    ShaderType_Mesh = 0,          // paper-design animated mesh gradient
    ShaderType_Dithering = 1,     // paper-design dithered procedural shapes
    ShaderType_GrainGradient = 2, // paper-design grain gradient ("Grainy")
    ShaderType_Warp = 3,          // paper-design warp
    ShaderType_Neuro = 4,         // paper-design neuro-noise ("Neuro")
    ShaderType_Simplex = 5,       // paper-design simplex-noise ("Simplex")
    ShaderType_Metaballs = 6,     // paper-design metaballs ("Metaballs")
    ShaderType_GodRays = 7,       // paper-design god-rays ("God Rays")
    ShaderType_Fluid = 8,         // radiant-shaders fluid-amber ("Fluid")
    ShaderType_Neon = 9,          // radiant-shaders neon-drip ("Wisp")
    ShaderType_Silk = 10,         // radiant-shaders silk-cascade ("Silk")
    ShaderType_Strata = 11,       // radiant-shaders painted-strata ("Strata")
    ShaderType_Custom = 12,       // user-supplied shader, runtime-compiled
} ShaderType;
