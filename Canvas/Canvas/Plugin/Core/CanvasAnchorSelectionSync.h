...
/// instantly but pollute cmd-Z; this trades instant-when-idle for a clean undo).
///
/// `writerTag` / `readerTag` identify the surface ("osc" / "mini") so a surface
/// never re-applies its own write.

/// Publish the current anchor selection for `layerID`.
void CanvasPublishAnchorSelection(NSString *writerTag, NSString *_Nullable layerID,
                                  NSIndexSet *indices);

/// The selection to apply if the OTHER surface published a newer one for
/// `layerID` since this reader last consumed; nil if unchanged / different layer
/// / this reader's own write.
NSIndexSet *_Nullable CanvasConsumeAnchorSelection(NSString *readerTag,
